...
def list_foods():
    ...
    cursor = foods.find().sort("name").skip(per_page * (page - 1)).limit(per_page)

    food_count = foods.count_documents({})
    ...
    return {
        "foods": [food(**doc).to_json() for doc in cursor],
        "_links": links,
    }
...
def new_food():
    ...
    food = food(**raw_food)
    insert_result = foods.insert_one(food.to_bson())
...
def get_food(slug):
    recipe = foods.find_one_or_404({"slug": slug})
...
def update_food(slug):
    ...
    updated_doc = foods.find_one_and_update(
        {"slug": slug},
        {"$set": food.to_bson()},
...
def delete_food(slug):
    deleted_food = foods.find_one_and_delete(
        {"slug": slug},
    )
    if deleted_food:
